...
void TFT_DisplayImage(const TFT_Config_t *Copy_TftDisplay, const u16 *Copy_Image)
{
    ...
    u8 xEndLow = (TFT_DISPLAY_WIDTH-1) & 0xFF;
...
static void TFT_SendCommand(const TFT_Config_t *Copy_TftDisplay, u8 Copy_Command)
{
    ...
    MCAL_GPIO_SetPinValue(Copy_TftDisplay->TFT_CSPin.TFT_Port, Copy_TftDisplay->TFT_CSPin.TFT_Pin, GPIO_LOW); 

    /**< Set DC (Data/Command Control) pin low to indicate command mode */ 
    MCAL_GPIO_SetPinValue(Copy_TftDisplay->TFT_DCPin.TFT_Port, Copy_TftDisplay->TFT_DCPin.TFT_Pin, GPIO_LOW); 

    /**< Perform SPI data transfer to send the command byte */ 
    SPI_Transfer(&Copy_Command, &Local_u8ReceivedData, 1); 

    /**< Set CS pin high to release the TFT display */ 
    MCAL_GPIO_SetPinValue(Copy_TftDisplay->TFT_CSPin.TFT_Port, Copy_TftDisplay->TFT_CSPin.TFT_Pin, GPIO_HIGH); 
}
...
static void TFT_SendData(const TFT_Config_t *Copy_TftDisplay, u8 Copy_Data)
{
    ...
    /**< Set DC (Data/Command Control) pin low to indicate command mode */ 
    MCAL_GPIO_SetPinValue(Copy_TftDisplay->TFT_DCPin.TFT_Port, Copy_TftDisplay->TFT_DCPin.TFT_Pin, GPIO_HIGH); 

    /**< Perform SPI data transfer to send the command byte */ 
    SPI_Transfer(&Copy_Data, &Local_u8ReceivedData, 1); 
